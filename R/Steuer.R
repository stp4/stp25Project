...
#' +4369981530117
#' 
#' grundsaetzlich bis 30. April des Folgejahres bis 30. Juni bei uebermittlung 
#' auf elektronischem Wege
#'
#' 
#' Branche: 73.2 Markt- und Meinungsforschung
#' Gewinnermittlungsarten
#' Vollständige Einnahmen-Ausgaben-Rechnung gemäß § 4 Abs. 3 => Ja
#' USt-Bruttosystem => Ja
#' 
...
#' erwarten laesst, sind diese Einkuenfte steuerlich unbeachtlich. 
#' Das heißt, dass Verluste aus
...
#' betrifft also vor allem Miethaeuser (siehe dazu das Infoblatt  Liebhaberei imSteuerrecht ).
#'  
#' 
#' 
#' Steuer(konto = "AT892050303302884626_2025-01-01_2025-12-31.xlsx",
#'       AfA = afa,
#'       konto_path = "C:/Users/wpete/luckycloud/2_Finanzen/Kontospiegel/2025/",
#'       #
#'       Versicherung.SVA = 2483.89,
#'       sva = c("Sozialvers.Anstalt"),
#'       #
#'       select=  c(Buchungsdatum = 1, Partnername =2, Betrag = 11)
#'  )
#' 
#' @param jahr Jahr
#' @param konto character, xls-Datei direkt aus der Bank exportiere Kontospiegel
#' @param Versicherung.SVA  numeric. Zahlungsbestätigung der SAV
#' @param AfA data.frame. mit der Funktion AfA() erzeugte Daten.
#' @param pauschale default 0.06  6 Prozent Pauschalierung fuer Kleinunternehmer
#' @param select Postitionen für c(Buchungsdatum = 1, Partnername =2, Betrag = 11)
#' @param konto_path Pfad zum Kontospiegel
#' @param Versicherung.SVA numeric. Zahlungsbestätigung der SAV
#' @param Reisespesen,Betriebsausgaben,Versicherungspraemien,Steuerberater numeric.
#' @param pattern_debit,privat,versicherung,kirche,spenden,sva character. pattern
#'
#' @return data.frame und speichern der Berechnungen als csv-daten 
#' @export
#' @importFrom lubridate year as_datetime as_date
#' @importFrom readxl read_excel
#' @importFrom stp25tools2 get_data render_f
...
#'       AfA = afa,
#'       konto_path = "C:/Users/wpete/luckycloud/2_Finanzen/Kontospiegel/2025/",
#'       #Zahlungsbestätigung der SAV
#'       Versicherung.SVA = 2483.89,
#'       #direkt aus der Bank exportiere Kontospiegel
#'       select=  c(Buchungsdatum = 1, Partnername =2, Betrag = 11)
#'  )
#'         
#' }
...
Steuer <- function(jahr = lubridate::year(lubridate::as_date(Sys.time())) - 1,
  ...
  knt <-
    ...
    make_konto(knt, jahr =jahr, select = select)
  print(head(knt))
  ...
  rslt <- data.frame(
    Pos = c(
      ...
      "Versicherung (SVA)",
      "Betriebsausgaben",
      paste0("Betriebsausgabenpauschale ", pauschale * 100, " Prz"),
  ...
  rslt$Eingaben.Ausgaben <-
    stp25tools2::render_f(rslt$Eingaben.Ausgaben, 2, decimal.mark = ",")
  rslt$Pauschale <- stp25tools2::render_f(rslt$Pauschale, 2, decimal.mark = ",")
